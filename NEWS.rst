...
* Add :ref:`spider-sub-command` design notes.
...
* Report *via* information for each module entry on JSON output of
  :subcmd:`avail`, :subcmd:`list` and :subcmd:`spider` sub-commands. *Via*
  information is the name of the loaded or interpreted module that enables the
  modulepath in which reported modules are stored.
